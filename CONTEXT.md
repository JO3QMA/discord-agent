...
_Avoid_: 改投稿, チャンク送信（用語として）, 再送（失敗リトライと混同する場合）, 文字数だけの途中切断を既定とすること, 1段落＝1通を既定とすること, 表やコードフェンスを通常段落と同じに途中切断すること, 経路ごとに違う分割規則, 長い本文の黙殺切り捨て（分割せず後半を捨てること）, 表の続きでヘッダ無しのデータ行だけを送ること, 分割したコードフェンスを開きっぱなしにすること, 装飾を足す前の長さだけで上限判定すること, 分割メッセージへの通し番号付与
