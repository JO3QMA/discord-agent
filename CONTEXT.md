...
**Skill**:
学習ループが持つ、再利用可能な手順（`SKILL.md`）と、そのトピックのオンデマンド事実（同じディレクトリの `NOTES.md` / `references/`）。agentskills.io 互換。MEMORY / USER が満杯でも、台帳をそこに圧縮して逃がさない。読書きは memory-skills MCP の `skill_view` / `skill_write_file` / `skill_patch(path=…)`。
_Avoid_: prompt template, custom command, plugin, SKILL.md 本文への台帳, MEMORY/USER へのリスト圧縮, 作業ディレクトリへの Write で skills を更新すること
...
**定期ジョブ**:
スケジュールで発火し、会話履歴に載せず結果をホーム（または指定チャンネル）へ届ける実行。セッション／実行ハンドルは会話と共有しない。`continuity` で前回出力を次プロンプトに注入し重複を避け、`mode: monitor`（continuity 併用）では同一／未変化応答の Discord 配信を省略でき、`notepad` でジョブ専用スクラッチパッドを跨ぎ実行できる。
...
**ターンキュー**:
ある会話で実行中のターンがあるあいだに届いた、まだ実行に入っていない Operator 発話の待ち行列。会話は待ちが空になるまで単一飛行（同時に走るターンは1つ）とし、そのあいだの新規発話は末尾に載る。現ターンは中断せず、終了後（成功・失敗を問わず）に先頭から一つずつ独立したターンとして処理する。待ち行列の要素は元の Discord メッセージ（本文・添付・送信 Operator・リアクション付け先）を保持する。`/stop` および `/new` では待ちを破棄する（破棄した旨を投稿し、待ち発話はキュー待ちリアクションから ❌ へ）。ターン中にセッションが消えた場合も待ちを破棄し、件数つきで告知する。件数上限は設けない。
_Avoid_: 割り込み結合, 中断して再結合, キュー全文の一括ターン, 本文テキストだけの待ち行列, `/stop` や `/new` のあと待ちだけ残して再開すること, 直前ターン失敗だけで待ちを止めること, セッション消滅後に待ちを続行すること, いま決める件数上限, 同一会話での並列ターン

**キュー待ちリアクション**:
ターンキューに載った Operator 発話に付く待ち表示（∞）。ターン実行開始時に外し、ターン状態リアクションへ引き継ぐ。`/stop` などで待ちのまま破棄されたときは ∞ を外して ❌ を付ける。
_Avoid_: ターン状態リアクションと同一視, 待ちのまま残す, 進捗メッセージ（⏳）

**ターン状態リアクション**:
チャット入口でターン実行に入った Operator 発話に付く、そのターンの状態表示。開始は 👀、本編の答えの送信成功で ☑、届けられなかったら（例外・送信失敗・セッション消滅・`/stop` 中断など）❌。常時 ON。スラッシュ・定期ジョブ・バックグラウンドターン、およびターンキュー待ちのあいだの後続発話には付けない（実行開始時に付ける）。進捗用の ⏳ メッセージとは別物で併存する。
_Avoid_: 進捗メッセージ（⏳）と同一視, 定着レビューの 💾 通知, 入口全体の統一ステータス（チャット以外まで同じ UX にする場合）, `/stop` を完了（☑）扱いすること, 設定で切る前提の任意機能（現状）, キュー待ちリアクション（∞）と同一視
...
**分割投稿**:
Discord の文字数上限を超える返信を、複数メッセージに分けて届けること。入口から出す長い本文（チャット返信・スラッシュ応答・ホーム／チャンネル送信・バックグラウンド結果など）は同一規則とする。区切りは段落・行を優先し、文字数での途中切断は最後の手段とする。上限に収まるあいだ、完成した段落／行を同一メッセージに載せる。表やコードフェンスは可能な限り一つの塊として扱い、途中で割らない。表を行で割るときは、続きのメッセージにもヘッダ行（と区切り行）を繰り返す。コードフェンスを行で割るときは、各メッセージでフェンスを開き直して閉じ、言語タグも引き継ぐ。各メッセージは、接頭辞やフェンス補修を含めた送信直前の最終文字列が Discord 本文上限に収まること。通数にも上限があり、超えた分は truncate する。
_Avoid_: 改投稿, チャンク送信（用語として）, 再送（失敗リトライと混同する場合）, 文字数だけの途中切断を既定とすること, 1段落＝1通を既定とすること, 表やコードフェンスを通常段落と同じに途中切断すること, 経路ごとに違う分割規則, 長い本文の黙殺切り捨て（分割せず後半を捨てること）, 表の続きでヘッダ無しのデータ行だけを送ること, 分割したコードフェンスを開きっぱなしにすること, 装飾を足す前の長さだけで上限判定すること, 分割メッセージへの通し番号付与, 通数上限なしの無制限連投
