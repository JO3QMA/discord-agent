...
**ターン状態リアクション**:
チャット入口でターン実行に入った Operator 発話に付く、そのターンの状態表示。開始は 👀、本編の答えの送信成功で ☑、届けられなかったら（例外・送信失敗・セッション消滅・`/stop` 中断など）❌。常時 ON。スラッシュ・定期ジョブ・バックグラウンドターン、および割り込みでキューに載っただけの後続発話には付けない。進捗用の ⏳ メッセージとは別物で併存する。
_Avoid_: 進捗メッセージ（⏳）と同一視, 定着レビューの 💾 通知, 入口全体の統一ステータス（チャット以外まで同じ UX にする場合）, `/stop` を完了（☑）扱いすること, 設定で切る前提の任意機能（現状）
